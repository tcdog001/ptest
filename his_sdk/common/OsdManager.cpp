...
HI_S32 OsdManager::EncodeFrame(HI_UNF_VIDEO_FRAME_INFO_S *pstFrame, HI_S32 EncodeType, HI_CHAR *DstFile)
{
 ...
 HI_S32 OsdManager::DrawCCText(HI_HANDLE hSurface, HI_RECT_S* pstRect, HI_CHAR* pTest,CCTEXT_ATTR_S * pstAttr)
{
        OSD_DISP_SURFACE_S*  pSurface = HI_NULL;
        SkBitmap   bitmap;
        ...
        SurfaceComposerClient::openGlobalTransaction();
        pSurface->surface->lock(&Buffer, NULL);
...
HI_S32 OsdManager::FillCCRect(HI_HANDLE hSurface, HI_RECT_S *pstRect, HI_U32 u32Color)
{
       OSD_DISP_SURFACE_S*  pSurface = HI_NULL;
        SkBitmap   bitmap;
        ANativeWindow_Buffer Buffer;
...
//translate
HI_S32 OsdManager::CCBlit(HI_HANDLE hSurface, HI_RECT_S *pstSrcRect, HI_RECT_S *pstDstRect)
{
        OSD_DISP_SURFACE_S*  pSurface = HI_NULL;
        SkBitmap   bitmap;
        ANativeWindow_Buffer Buffer;
...
//translate and scale
HI_S32 OsdManager::CaptionDrawBitmap(HI_HANDLE hSurface, HI_U8 *bitmapBuffer,HI_RECT_S *pstSrcRect,HI_RECT_S *pstDstRect)
{
    OSD_DISP_SURFACE_S*  pSurface = HI_NULL;
    SkBitmap   bitmap;
    SkBitmap   tempBitmap;
    ...
    if (pSurface == HI_NULL)
    {
     ...
     canvas.save();
     canvas.scale(sx, sy);
     canvas.drawBitmap(tempBitmap, pstDstRect->s32X, pstDstRect->s32Y, &paint);
     canvas.restore();
