...
{
    HI_UNF_VENC_STREAM_S stVencStream;  /**<video stream encoded data *//**<CNcomment:视频流编码数据 */
    HI_UNF_ES_BUF_S      stAencStream;            /**<audio stream encoded data *//**<CNcomment:音频流编码数据 */
}HI_TRAN_DATA_TYPE_U;
...
typedef struct tagTran_STREAM_S
{
    ...
    HI_TRAN_DATA_TYPE_U  enDataStream;             /**<stream data *//**<CNcomment:媒体流数据*/
}TRANSCODER_STRAM;
...
//    HI_S32 s32Y;                                            /**<Y coordinate*//**<CNcomment:转码的起始位置 Y 坐标*/
    ...
    HI_S32 s32VideoBitRate;                /**<video bitrate*//**<CNcomment:视频比特率 */
...
/**-----Standard Functions--------*/
/**
\brief transcoder module initialization function.CNcomment:初始化转码器
\attention \n
HiTranscoder module must init function.CNcomment:调用转码要求首先调用该接口。
...
/**-----Standard Functions--------*/
/**
\brief transcoder  uninitialization function.CNcomment:去初始化转码器
\attention \n
HiTranscoder module all clear function must called behind.CNcomment:转码退出要求最后调用该接口。
...
/**-----Standard Functions--------*/
/**
\brief create transcoder function .CNcomment:打开转码通道
\attention \n
...
\param[in] pstAttr transcoder handle attr.CNcomment:设置转码通道的属性。
\param[out] pHandle  created the handle of transcoder.CNcomment:建立的转码通道句柄。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
...
/**-----Standard Functions--------*/
/**
\brief destroy transcoder handle.CNcomment:模块销毁transcoder句柄
\attention \n
called this function,need destroy all protocol.
CNcomment:当业务需要关闭转码能力时调用，需要注意此时的状态需要销毁所有的协议，虚拟驱动。

\param[in] pHandle created transcoder handle.CNcomment:已创建的transcoder句柄。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
...
/**-----Standard Functions--------*/
/**
\brief get the attr of transcoder handle.CNcomment:获取转码能力属性
\attention \n
this function called after transcoder create handle function.
CNcomment:当业务需要获取转码能力属性，需要注意此接口需要在create函数调用后使用。

\param[in] pHandle created transcoder handle.CNcomment:已创建的transcoder句柄。
\param[in] eTranAttr attribute set type.CNcomment:指定返回的属性集类型。
\param[out] pstAttr returned attribute set content.CNcomment:指定返回的属性集内容。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. get attribute error happened.CNcomment:获取属性集失败。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief set transcoder handle attr.CNcomment:设置转码能力属性
\attention \n
need called after transcoder create function.
CNcomment:当业务需要设置转码能力属性需要注意此接口需要在create函数调用后使用。

\param[in] pHandle the handle who need set attr.CNcomment:已创建的transcoder句柄。
\param[in] eTranAttr attribute set type.CNcomment:指定返回的属性集类型。
\param[in] pstAttr the attr will set to handle.CNcomment:设置转码通道的属性。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. set attribute error happened.CNcomment:设置属性集失败。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief get the resource info of transcoder module.CNcomment:获取转码使用资源情况
\attention \n
this function called after transcoder initial function.
CNcomment:当业务需要获取转码资源情况需要注意此接口需要在init函数调用后使用。

\param[in] pHandle the handle who need set attr.CNcomment:已创建的transcoder句柄。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. GetResourceInfo error happened.CNcomment:获取资源信息失败。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief get the resource info of transcoder module.CNcomment:获取转码Debug 信息
\attention \n
this function called after transcoder initial function.
CNcomment:当业务需要获取转码Debug 信息需要注意此接口需要在init函数调用后使用。

\param[in] pHandle the handle who need set attr.CNcomment:已创建的transcoder句柄。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. GetDebugInfo error happened.CNcomment:获取Debug信息失败。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief set transcoder handle status --- start.CNcomment:设置转码器状态机－－start
\attention \n
when transcoder enter stop-start status machine ,after transcoder create handle.
CNcomment:在转码器进入Stop-start状态接收该接口的状态机转换请求。

\param[in] hTranHandle created transcoder handle.CNcomment:已创建的transcoder句柄。
\param[in] bBlock blocked called or not .CNcomment:是否阻塞。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. transcoder start error happened.CNcomment:转码启动失败。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief set transcoder handle status --- stop.CNcomment:设置转码器状态机－－stop
\attention \n
...
\param[in] bBlock blocked called or not .CNcomment:是否阻塞。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. transcoder stop error happened.CNcomment:转码关闭失败。
\see \n
...
其回调流程根据 Command不同而由用户指定不同的实现分支
\attention \n
transcoder enter stop-start status machine ,after transcoder create handle.
CNcomment:在转码器进入Stop-start状态接收该接口的状态机转换请求。

\param[in] hTranHandle created transcoder handle.CNcomment:已创建的transcoder句柄。
\param[in] u32Command transcoder callback command.CNcomment:转码回调的指令。
\param[in] pstAttr callback attr .CNcomment:指令的参数。
\param[in] s32AttrLen callback attr len.CNcomment: 指令参数的长度。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. SetCommand error happened.CNcomment:回调caller失败。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief register media data read handle.CNcomment:注册媒体数据读取句柄
\attention \n
register media data read handle,this function called by protocol and user.
CNcomment:注册媒体数据读出接口，该接口由protocol模块进行调用。

\param[in] hTranHandle created transcoder handle.CNcomment:已创建的transcoder句柄。
\param[out] phMbufReadHandle media data read handle.CNcomment:媒体数据的读取句柄。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. Register media Handle error happened.CNcomment:注册媒体读写句柄失败。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief unregister media data read handle.CNcomment:去注册媒体数据读取句柄
\attention \n
unregister media data read handle,this function called by protocol and user.
CNcomment:去注册媒体数据读出接口，该接口由protocol模块进行调用。

\param[in] phMbufReadHandle registed media data read handle.CNcomment:已注册的媒体数据的读取句柄。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. DeRegister media Handle error happened.CNcomment:去注册媒体读写句柄失败。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief read media data from mbuf handle.CNcomment:读取媒体数据
\attention \n
when there no data in buffer, will return HI_FAILURE.
CNcomment:读取的数据可能由于MBUF数据为空，无法读出，此时返回HI_FAILURE。

\param[in] hReadHandle registed media data read handle.CNcomment:已注册的媒体数据的读取句柄。
\param[out] pstTranFrame returned media data from read handle.CNcomment:从MBUF中读取的数据帧结构。
\retval 0  HI_SUCCESS Success.CNcomment:成功返回。
\retval ::HI_FAILURE  -1. read media stream error happened.CNcomment:读取媒体数据返回空。
\see \n
...
/**-----Standard Functions--------*/
/**
\brief get the default attribute set of transcoder.CNcomment:获取转码默认属性集
\attention \n

\param[out] stTranHandleAttr returned default transcoder attribute set.CNcomment:返回转码的默认属性集。
