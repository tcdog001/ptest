...
\param[in] u64Address       open address CNcomment:打开地址(当分区名无效即pPartitionName为空时使用)CNend
\param[in] u64Len           open length CNcomment:打开长度(当分区名无效即pPartitionName为空时使用)CNend
...
\param[in] u32Flags     OOB flag  CNcomment:可取值 HI_FLASH_RW_FLAG_WITH_OOB，表示数据内容是否带 OOB 区CNend
\retval ::TotalRead     read flash vaild length
\retval ::HI_FLASH_END_DUETO_BADBLOCK    CNcomment:读Flash遇到坏块结束(读完有效长度数据后,返回该值,由用户判断此种情况成功与否)CNend
\retval ::HI_FAILURE    failure
\see \n
*/
